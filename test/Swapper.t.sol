...
contract SwapperTest is Test {
    ...
    function testSwap() public {
        ...
        assertTrue(
            ERC20(usdc).balanceOf(address(swapper)) == 0,
            "Check swapper balance usdc"
        );
        assertTrue(
            ERC20(weth).balanceOf(address(swapper)) == 0,
            "Check swapper balance weth"
        );
        assertTrue(
            ERC20(usdc).balanceOf(address(this)) == initialBalanceUSDC - amount,
            "Check  balance usdc"
        );

        assertTrue(
            ERC20(weth).balanceOf(address(this)) ==
                initialBalanceWETH + amountOut,
            "Check  balance weth"
        );
